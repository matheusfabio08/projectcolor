-- =============================================
-- PROJECTCOLOR — Schema PostgreSQL consolidado
-- Baseado no schema atual + patches de compatibilidade
-- =============================================
...
CREATE TABLE IF NOT EXISTS production_orders (
  id                      SERIAL PRIMARY KEY,
  sheet_id                INTEGER REFERENCES production_sheets(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS batch_ops (
  id              SERIAL PRIMARY KEY,
  batch_id        INTEGER REFERENCES preparation_batches(id) ON DELETE CASCADE,
  op_id           INTEGER REFERENCES production_orders(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS po_box4 (
  id                 SERIAL PRIMARY KEY,
  po_id              INTEGER REFERENCES production_orders(id) ON DELETE CASCADE,
  employee_id        TEXT,
  has_adjustment     BOOLEAN DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS lista_saida (
  id                SERIAL PRIMARY KEY,
  op_id             INTEGER NOT NULL REFERENCES production_orders(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_production_orders_op_number ON production_orders(op_number);
...
CREATE INDEX IF NOT EXISTS idx_po_in_progress_op_stage ON po_in_progress(op_id, stage);
...
CREATE INDEX IF NOT EXISTS idx_lista_saida_exit_date ON lista_saida(exit_date);
CREATE INDEX IF NOT EXISTS idx_fabric_quality_status ON fabric_quality_inspections(status);

-- Seeds mínimos de compatibilidade
INSERT INTO fibras (name, is_active)
SELECT nome, TRUE FROM (
  VALUES
    ('POLIÉSTER'),
    ('ALGODÃO'),
    ('VISCOSE'),
...
WHERE NOT EXISTS (SELECT 1 FROM fibras LIMIT 1);

INSERT INTO transportadoras (name, is_active)
SELECT nome, TRUE FROM (
  VALUES
    ('PRÓPRIA'),
    ('TERCEIRIZADA')
...
INSERT INTO regioes_entrega (name, is_active)
SELECT nome, TRUE FROM (
  VALUES
    ('JARAGUÁ'),
    ('BRUSQUE'),
...
WHERE NOT EXISTS (SELECT 1 FROM regioes_entrega LIMIT 1);
